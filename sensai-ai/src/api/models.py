...
class CreatePostRequest(BaseModel):
    ...
    content: str
    post_type: Literal["thread", "question", "note", "reply", "poll"]
    parent_id: Optional[int] = None
    # Poll-specific fields
    poll_options: Optional[List[str]] = None
    poll_duration_days: Optional[int] = 7
...
class Post(BaseModel):
    ...
    # Poll-specific fields
    poll_options: Optional[List[str]] = None
    poll_votes: Optional[Dict[str, int]] = None  # option -> vote count
    poll_duration_days: Optional[int] = None
    allow_multiple_answers: Optional[bool] = None
    poll_expires_at: Optional[datetime] = None
    user_poll_votes: Optional[List[str]] = None  # options the current user voted for
    # QnA-specific fields
...
# Enhanced Learning Hubs & Forums Models

class ModerationAction(BaseModel):
    id: int
    post_id: int
    moderator_id: int
    action_type: Literal["hide", "delete", "warn", "ban", "approve", "flag"]
    reason: str
    created_at: datetime
    is_ai_moderated: bool = False
...
class PostReport(BaseModel):
    ...
    post_id: int
    reporter_id: int
    reason: Literal["spam", "harassment", "inappropriate", "misinformation", "other"]
    description: Optional[str] = None
...
class UserReputation(BaseModel):
    user_id: int
    hub_id: Optional[int] = None  # None for global reputation
    score: int = 0
...
class LinkPostRequest(BaseModel):
    post_id: int
    link_type: Literal["task", "skill", "badge"]
    link_id: int  # ID of the task, skill, or badge
...
class SubscribeHubRequest(BaseModel):
    user_id: int
    hub_id: int
...
class SearchRequest(BaseModel):
    ...
    sort_by: Literal["relevance", "date", "votes", "replies"] = "relevance"
    limit: int = 20
    offset: int = 0
...
class PersonalizedFeedRequest(BaseModel):
    ...
    limit: int = 20
    offset: int = 0
    feed_type: Literal["following", "subscribed_hubs", "recommended", "trending"] = "recommended"


class AIModerationResult(BaseModel):
    is_toxic: bool
    toxicity_score: float  # 0.0 to 1.0
    categories: List[str]  # ["harassment", "spam", "inappropriate", etc.]
    suggested_action: Literal["approve", "flag", "hide", "delete"]
    explanation: str
    requires_human_review: bool = False


class EnhancedHub(Hub):
    subscriber_count: int = 0
    post_count: int = 0
    active_today: int = 0
    topics: List[str] = []
    moderators: List[int] = []  # User IDs of moderators
...
class EnhancedPost(Post):
    ...
    linked_badges: List[Dict[str, Any]] = []
