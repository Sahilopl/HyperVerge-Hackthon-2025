...
# Handle Windows aiohttp import issues
try:
    import aiohttp
except (ImportError, KeyboardInterrupt) as e:
    # Fallback for Windows issues with aiohttp
    print(f"Warning: aiohttp import issue: {e}")
    aiohttp = None


async def send_slack_notification(message: Dict, webhook_url: str):
    if not aiohttp:
        print("Slack notification skipped - aiohttp not available")
        return
