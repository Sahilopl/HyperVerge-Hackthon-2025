    ...
    async jwt({ token, user, account }: { token: JWT; user?: User; account?: Account | null }) {
      ...
      if (account && user) {
        ...
        if (account.provider === 'google') {
          try {
            const result = await registerUserWithBackend(user as any, account);
            ...
            if (result && result.id) {
              ...
              // Fallback: use Google's user ID or email hash as temporary ID
              token.userId = user.id || btoa(user.email || '').replace(/=/g, '');
            }
          } catch (error) {
