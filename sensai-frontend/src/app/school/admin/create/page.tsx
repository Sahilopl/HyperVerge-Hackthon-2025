...
export default function CreateSchool() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Temporary bypass for development - check if we have user data at all
        const userEmail = user?.email;
        const tempUserId = user?.id || user?.email || 'temp-user-' + Date.now();

        if (!tempUserId) {
            console.error("User not authenticated - no ID or email available");
            alert("Please sign in with Google first to create a school.");
            return;
        }

        console.log("User authenticated with temp ID:", tempUserId);
